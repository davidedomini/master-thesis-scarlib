...
% DQN, nello specifico, è la versione deep dell'algoritmo Q-Learning, quindi produce come output un Q-Value per ogni 
%     possibile azione [ref fig]. Questo fa sì che l'addestramento della rete neurale utilizzata sia un task di regressione in cui 
%     può essere utilizzata come funzione di loss una square error loss: 
%     $$ L = (y_t - \hat{y_t})^2 $$
%     Dove $y_t$ è il valore reale e $\hat{y_t}$ è il valore predetto al tempo t. Siccome RL è un apprendimento di tipo 
%     non-supervisionato e, quindi, non si hanno label il valore $y_t$ viene stimato utilizzando l'equazione di Bellman,
%     portando la funzione di loss, per una transizione $<s_t, a_t, s_{t+1}, r_{t+1}>$, a diventare:
