...
The growing popularity of \emph{highly distributed IoT} has highlighted the need for new methods to develop these 
    ...
    \begin{color}{red}
    (i, \emph{partial observability}), each entity/agent posses only a partial view of the environment in 
        which it operates;
    (ii, \emph{full distribution}), there is no central entity that coordinates the entire system, as in 
    ...
    (iii, \emph{uncertainty}), each entity/agent is influenced by its interactions with the environment and with other 
...
\begin{color}{red} This thesis proposes the starting point for a \emph{hybrid toolchain} \end{color} that aims to exploit the potential of both aggregate computing
...
\begin{color}{red}
Finally, two experiments are also presented to validate the framework's functionalities by testing it in basic 
...
Significant technological advancements have paved the way for the emergence of a field known as \emph{collective computing} 
    ...
    and \emph{self-organization} \cite{schmeck2011organic}, leading us to 
...
On the other hand, approaches exists that leverage various artificial intelligence (AI) techniques, 
    ...
    (i.e., the environment constantly changes) \cite{hernandez2017survey}. \end{color} 
...
\emph{Cyber-physical swarms (CPSWs}) are an extension of swarm systems in which both logical and
    ...
    complex collective behavior emerges that leads to the resolution of the given task. \end{color}Examples of these systems (\Cref{fig:cpsw}) are a fleet of drones tasked with monitoring a park to 
...
In science and engineering, the use of \emph{simulators} plays a key role. These simulators enable the experimentation 
    ...
    \begin{color}{red}
    Moreover, simulation is crucial in MARL with offline learning, enabling agents to learn a model in a controlled 
    environment without relying on physical components, such as robots, that may be cost-prohibitive or unavailable.
    \end{color}
...
The first scenario (\Cref{fig:cpsw1}) represents one of the simplest conceivable instances for CPSWs, in which the proposed hybrid 
    ...
    \begin{color}{red}
    The goal is to define a policy that allows the fleet to identify the adverse event and coordinate to manage it within a 
    given time frame.
    \end{color}
...
class  FollowTheLeaderRewardFunction extends RewardFunction {
    ...
    override def compute(currentState: State, action: Action, nextState: State): Double = {
        ...
        AgentGlobalStore().put(s.agentId, "distance", distanceFactor)
        AgentGlobalStore().put(s.agentId, "collision", collision)
        AgentGlobalStore().put(s.agentId, "reward", collision + distanceFactor)
