...
% L'architttura di scafi è composta da vari moduli (Figura 2.5), i principali sono: 
%     i) scafi-core, contiene il DSL e una "libreria standard" di funzioni riusabili 
%         (e.g., Gradients, BlockG, BlockS, e altre),
%     ii) scafi-stdlib-ext, fornisce una serie di funzioni aggiuntive, siccome queste richiedono dipendenze
%         esterne è tenuta separata dal core,
%     iii) scafi-simulator, fornisce un supporto per simulare sistemi aggregati, 
%     iv) scafi-simulator-gui, fornisce una gui per visualizzare ed interagire con la simulazione,
%     v) spala, fornsice un middleware actor-based per AC fondato su Akka
%     vi) scala-distributed, fornisce un integration-layer per spala in ScaFi.
