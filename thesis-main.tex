...
% Questo framework è stato progettato per supportare lo sviluppo di sistemi CMARL in ambienti JVM-based 
%     attraverso delle specifiche ad alto livello. 
%     Per arrivare a questo obiettivo il tool è stato diviso in tre moduli principali, ovvero:
%     i) scarlib-core: implementa i concetti principali del dominio CMARL astraendo dai dettagli 
%         implementativi di basso livello,
%     ii) dsl-core: fornisce un linguaggio ad alto livello per la specifica di un sistema CMARL, e 
%     iii) alchemist-scafi: fornisce i bindings fra scarlib e i due tool Alchemist e ScaFi in modo da poter
%         eseguire esperimenti in un ambiente simulato sfruttando aggregate computing.

% La modularizzazione del framework porta diversi vantaggi, da un lato permette all'utente finale di poter utilizzare
%     solo i componenti che gli sono realmente utili, dall'altro permette di poter estendere e modificare il framework 
%     in modo semplice. Ad esempio, ScaRLib, per via degli attuali bisogni, ha già implementato il modulo alchemist-scafi,
%     se un utente volesse utilizzare un simulatore diverso da alchemist (e.g., FlameGPU) sarebbe sufficiente non importare
%     tale modulo ed implementare un nuovo environment personalizzato che si basi su tale simulatore.
...
The framework has been devised to aid the development of CMARL systems in JVM-based environments through high-level 
    ...
    a learning engine used to conduct training and optimization of neural networks utilised by the RL algorithms. 
    It's worth noting that, if required, an alternative substitute module that has similar functionality can be used instead.
