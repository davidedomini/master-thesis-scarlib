...
The growing popularity of \emph{highly distributed IoT} has highlighted the need for new methods to develop these 
    systems effectively and at scale. 
    Key distinguishing features of these systems include:
    (\emph{partial observability}) each entity/agent posses only a partial view of the environment in 
        which it operates;
    (\emph{full distribution}) there is no central entity that coordinates the entire system, as in 
        traditional client-server architectures (instead, computation takes place directly on the IoT 
        device or on some edge devices distributed throughout the system, near the IoT devices);
    (\emph{uncertainty}) each entity/agent is influenced by its interactions with the environment and with other 
        agents, introducing a level of stochasticity into the system.
Over the years, numerous methods have been suggested to address these challenges, including:
    \emph{Aggregate Computing} \cite{VIROLI2019100486}, a macro-programming paradigm, 
    and \emph{Multi-Agent Reinforcement Learning} \cite{busoniu2008comprehensive, marlsurvey}, a machine learning paradigm.
    This thesis proposes the starting point for a \emph{hybrid toolchain} that aims to exploit the potential of both aggregate computing
    and multi-agent reinforcement learning to develop systems capable of learning from experience and 
    self-organizing in case of changes in the external environment.

To attain this objective, we present \emph{ScaRLib} \cite{scarlib}, a framework designed to streamline the creation of these systems in 
...
Finally, two experiments are also presented to validate the framework functionalities by testing it in basic 
    contexts specific to this domain. These experiments were beneficial in verifying the proper functioning 
    of the tool and highlighting its strengths, as well as identifying areas for future work.
\end{abstract}
...
Finally, I would like to thank all my university colleagues and friends who have stood by my side: 
    ...
    Enrico, Giulio, Carla, Angela, Chiara, Maria, Chris, Luca and many others.
...
Significant technological advancements have paved the way for the emergence of a field known as \emph{collective computing} 
    ...
    and \emph{self-organization} \cite{schmeck2011organic}: hence we shall
...
On the other hand, approaches exist that leverage various artificial intelligence (AI) techniques, 
...
Finally, the last goal of this thesis is to validate the functionalities of the framework by implementing 
    two common experiments in this domain; in this way it will be possible to understand the current state of the tool 
...
\Cref{chap:introduction,chap:background} presents a broad scope of the objectives, motivations, and theoretical concepts that serve as the foundation 
    ...
    Finally, in \Cref{chap:validation,chap:conclusions}, two experiments carried out to validate the functionalities of the framework are presented,
...
\emph{Cyber-physical swarms (CPSWs}) are an extension of swarm systems in which both logical and
    ...
    complex collective behavior emerges that leads to the resolution of the given task. Examples of these systems (\Cref{fig:cpsw}) are a fleet of drones tasked with monitoring a park to 
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/scafi-web-example-2.png}
...
\emph{Aggregate computing} \cite{AC} is a macro-programming model whose foundation is based on \emph{field calculus}, providing innovative solutions
...
\Cref{fig:channel-scafiweb} shows the execution of the channel algorithm on ScaFiWeb.
...
\begin{figure*}[h!]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/channel-scafiweb-1.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/channel-scafiweb-2.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/channel-scafiweb-3.png}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
        \includegraphics[width=\textwidth]{figures/channel-scafiweb-4.png}
    \end{subfigure}
...
Another point to pay attention to is the system by which the training and execution of the various agents are carried out. 
    ...
    Thus, MARL algorithms can be categorized into three types (\Cref{fig:exmod}):
    \begin{enumerate}
...
The first scenario (\Cref{fig:cpsw1}) represents one of the simplest conceivable instances for CPSWs, in which the proposed hybrid 
    ...
    This first scenario will be implemented in \Cref{chap:validation} to validate the framework.

    The second example (\Cref{fig:cpsw2,fig:cpsw21}) also involves a fleet of drones but is more intricate than the previous one. In this case, 
...
A final example (\Cref{fig:cpsw3}) entails the use of \emph{wearable devices} (e.g., smartwatches) for \emph{crowd management} during a public event 
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \caption{Adversial events monitoring.}
        %\fonte{\url{https://tinyurl.com/y33s6scu}}
        \label{fig:cpsw21}
    \end{subfigure}
    \begin{subfigure}[b]{0.49\textwidth}
        \centering
...
\begin{figure*}[t]
    ...
    \begin{subfigure}[b]{0.25\textwidth}
        \centering
        \fbox{\includegraphics[width=\textwidth]{figures/ftl1.pdf}}
        \caption{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.25\textwidth}
        \centering
        \fbox{\includegraphics[width=\textwidth]{figures/ftl2.pdf}}
        \caption{}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.25\textwidth}
        \centering
        \fbox{\includegraphics[width=\textwidth]{figures/ftl3.pdf}}
        \caption{}
    \end{subfigure}
\caption{Snapshots of the learned policy, the time flow is from left to right.}
...
This section discusses the current state of the framework, highlighting its strengths and weaknesses
    that will shape future work in \Cref{chap:conclusions}.
